...
class GenvexDevice extends EventEmitter {
  ...
  async poll() {
    ...
    try {
      // Read datapoints
      const dpReq = getDatapointRequestList();
      const dpResults = await this.connection.readDatapoints(dpReq.keys, dpReq.requests);

      for (const [key, rawValue] of dpResults) {
        const register = Optima270Datapoints[key];
        if (register) {
          const value = convertDatapointValue(rawValue, register);
      ...
      // Read setpoints
      const spReq = getSetpointRequestList();
      const spResults = await this.connection.readSetpoints(spReq.keys, spReq.requests);

      for (const [key, rawValue] of spResults) {
        const register = Optima270Setpoints[key];
        if (register) {
          const value = convertSetpointValue(rawValue, register);
          const oldValue = this.data.get(register.name);
  ...
  async setValue(name, value) {
    const register = getSetpointByName(name);
    ...
    const rawValue = toRawSetpointValue(value, register);
